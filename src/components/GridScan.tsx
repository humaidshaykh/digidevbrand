...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  // Memoized sensitivity calculations
  ...
  useEffect(() => {
    ...
    renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, 1.5)); // Cap pixel ratio
    ...
    const tick = () => {
      ...
      const skew = new THREE.Vector2(
        lookCurrent.current.x * sensitivityParams.skewScale, 
        -lookCurrent.current.y * sensitivityParams.yBoost * sensitivityParams.skewScale
      );
